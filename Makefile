...
.PHONY: default help clean shebang perms symlink setup test distclean install-local uninstall-local
...
install-local: setup
	...
	@ln -sfn "$(abspath pf_universal)" "/usr/local/bin/pf"
	@echo "Installed symlink: /usr/local/bin/pf -> $(abspath pf_universal)"
