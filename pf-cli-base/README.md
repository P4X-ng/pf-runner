...
cp dist/pf /usr/local/bin/ # this is a static executable

That's it!



## Build and install (for people that like building)

### Install dependencies

Either use the venv bundled in this repo - you end up with a static executable, so no need to
keep dependencies around if you don't want them - or install them in your environment with:
...
thisThe below will build and drop a symlink in ~/.local/bin, not the recommended way to do it, 
but it works if you just want to try it out.
...
or to build the static executable
...
and the static executable will be built and moved to /usr/local/bin/, just make sure your sudo command uses your venv as well.
